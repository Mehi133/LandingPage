...
interface ActiveSalesComparablesChartProps {
  activeListings?: any[];
...
const ActiveSalesComparablesChart: React.FC<ActiveSalesComparablesChartProps> = ({ 
  activeListings = [], 
  suggestedPrice,
  marketData,
  pricingData 
  ...
  // Process active listings data for chart
  const chartData = activeListings.slice(0, 8).map((listing, index) => {
    const price = typeof listing.price === 'string' 
      ? parseFloat(listing.price.replace(/[$,]/g, '')) || 0
      : listing.price || 0;
    
    const address = listing.address || `Property ${index + 1}`;
    ...
    return (
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold text-foreground">Active Sales Comparables</CardTitle>
        </CardHeader>
        <CardContent className="pt-0">
          <p className="text-sm text-muted-foreground">No active listings data available for comparison.</p>
        </CardContent>
      </Card>
    );
  }
...
export default ActiveSalesComparablesChart;
